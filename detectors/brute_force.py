...
DB_PATH = Path("threatlens/db/events.db")
...
def build_alert(ip: str, matching_events: list[dict], window_hits: list[datetime]) -> dict:
    return {
        ...
        "source_ip":   ip,
        "description": (
            f"Brute force detected: {ip} made {len(window_hits)} failed login "
            f"attempts within {WINDOW_SECS}s. "
            f"First: {window_hits[0].isoformat()}  Last: {window_hits[-1].isoformat()}"
        ),
        "event_count": len(window_hits),
        "window_secs": WINDOW_SECS,
        "attack_id":   "T1110",
        "attack_name": "Brute Force",
        "raw_events":  [e["raw"] for e in matching_events[:THRESHOLD]],
    }
